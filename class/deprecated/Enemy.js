(function () {
    ...
    var Enemy = function (stage, params) {
        this.max_speed  = 4;
        this.speed 		= 2;
        this.jump_speed = 24;
    ...
    var p = Enemy.prototype = new global.NPC();
    ...
    p.init_contactable = function (){
        ...
        this.contactable.setCategory(fixture,['ENEMY'])
    };
    ...
    /**
     * основая цель ходить
     * 1: ходить и искать игрока
     * 2: если нашел то итди к нему
     * 3: убить персонажа если убил вернутся к задаче 1
     */
    p.init_AI = function () {
        this.tasks = enemyTasks;

        var tasksList = [
            enemyTasks.enemyTaskWalk    ,global.WalkEnemyTask,
            enemyTasks.enemyTaskAttack  ,global.AttackEnemyTask,
            enemyTasks.enemyTaskTarget  ,global.TargetEnemyTask
        ];

        this.taskManager = new global.TaskManager(this, tasksList, true, function () {
            console.log('Персонаж потерял смысл жизни убить его');
        });

        this.taskManager.addTask(this.tasks.enemyTaskWalk);
    };
    ...
    p.init_FSM = function () {
        ...
        var statesList = [
            enemyStates.kStateIdle,     global.EnemyIdleState,
            enemyStates.kStateWalk,     global.EnemyWalkState,
            enemyStates.kStateJump,     global.EnemyJumpState,
            enemyStates.kStateFall,     global.EnemyFallState,
            enemyStates.kStateDead,     global.EnemyDeadState,
            enemyStates.kStateDamaged,  global.EnemyDamagedState
        ];

        this.fsm = new global.FSM(this, statesList);
        this.fsm.enter(enemyStates.kStateIdle);
    };
